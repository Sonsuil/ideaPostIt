...
export default function Board() {
  ...
  const fetchPostits = async () => {
    let url = `/api/postits?board_id=${encodeURIComponent(boardId)}`;
  ...
  const handleSavePostit = async (e: React.FormEvent) => {
    ...
    if (editPostitId !== null) {
      ...
      const res = await fetch("/api/postits", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ content: newContent, color: newColor, folder_id: targetFolderId, pos_x, pos_y, board_id: boardId }),
      });
  ...
  const handleAddFolder = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/folders", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: newFolderName, pos_x: 100, pos_y: 100, board_id: boardId }),
    });
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/auth", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ boardId, password: passwordInput }),
      });
    ...
    return (
      <div style={{ width: '100vw', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--background)' }}>
        <form onSubmit={handleAuthSubmit} style={{ background: 'var(--surface)', padding: '40px', borderRadius: '16px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '320px' }}>
          ...
          <h2 style={{ marginBottom: '8px', fontSize: '20px', fontWeight: 'bold' }}>{boardId.toUpperCase()} 게시판</h2>
          <p style={{ color: 'var(--text2)', marginBottom: '24px', fontSize: '14px' }}>비밀번호를 입력해주세요.</p>
